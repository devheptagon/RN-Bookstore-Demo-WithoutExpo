...
export const buildQueryUrl = (keyword, page) => {
  ...
  return `${apiBaseUrl}?q=intitle:${keyword}&startIndex=${ (page-1) * pageSize }&maxResults=${pageSize}`
}
...
export const buildDTOList = (data) => {
  ...
  data.map(d => {
    ...
    const { imageLinks, title, subtitle, authors, publishedDate, averageRating, description, industryIdentifiers } = volumeInfo
    const { listPrice } = saleInfo  
    const isbn = industryIdentifiers && industryIdentifiers.length && industryIdentifiers[0]
    result.push({ 
      id,
      volumeInfo: { imageLinks, title, subtitle, authors, publishedDate, averageRating },
      saleInfo: { listPrice },
      description,
      isbn
    })
